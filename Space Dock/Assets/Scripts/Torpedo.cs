...
public class Torpedo : MonoBehaviour {
    ...
    void alignToTarget()
    {
        ...
        {
            // if the player is the target, make sure the player knows he is sensor locked
            if(target.GetComponent<PlayerShip>())
            {
              target.GetComponent<PlayerShip>().setTargeted(true);
            }
    ...
    public void destroy(Vector3 velocity)
    {
        ...
        if(target.GetComponent<PlayerShip>())
        {
          // if target is player, no longer detect player ship once this torpedo is destroyed
          target.GetComponent<PlayerShip>().setTargeted(false);
        }
